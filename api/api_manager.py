...
class ApiManager:
	...
	def getInsertImageToSmartstoreList(self, params):
		try:
			user_id = params.get("user_id", None)
			url = ConfigCommon.HOST_ADDR + "/index.php?c=API&sc=GET_INSERT_IMAGE_TO_SMARTSTORE_LIST&proc=1"
	...
	def updateInsertImageToSmartstoreList(self, params):
		try:
			user_id = params.get("user_id", None)
			inserted_image_list = params.get("inserted_image_list", [])
			url = ConfigCommon.HOST_ADDR + "/index.php?c=API&sc=UPDATE_INSERT_IMAGE_TO_SMARTSTORE_LIST&proc=1"
